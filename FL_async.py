...
def SFL_over_SA(rule_iid ,K, Group):
    ...
    recv_iter_last = [0 for i in range(args.num_users)]
    ...
    for idx, clnt in enumerate(user_list):    #按照星历表遍历       
        ...
        recv_iter_list.append(sats_clnt.recv_iter)
        ...
        if len(w_locals_client) >= args.K:             #iter%2==0,5,10
            ...
            if args.Group == 1:
                ...
                FL_params = FedBuff(w_old, w_locals_client, args.local_lr)
            #清除Buff-----------------------------
            recv_iter_list = []
            w_locals_client = []
            if args.Group == 1:
        ...
        recv_iter_last[clnt] = server_iter
