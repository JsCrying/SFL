...
def SFL_over_SA(rule_iid ,K, Group):
    ...
    """
    data_csv = pd.read_csv('./sort_EndTime_SH_550km_5PLAN_40SAT_40days.csv')#按照EndTime排序
    ...
    # test_data = data_obj.test_x, #测试集
    # # test_data = data_obj.test_x[0], #测试集
    ...
    for idx, clnt in enumerate(user_list):    #按照星历表遍历       
        ...
        need_ffc = False #没有加入额外的卷积层了，保持False
        ...
        if idx == 0 : #先测初始值
            ...
            [loss_tst, acc_tst] = local.evaluate(net = copy.deepcopy(FL_model).to(device), 
                            ell = args.local_ep , dataset_tst = data_obj.test_x, 
                            dataset_tst_label = data_obj.test_y , net_server = net_server)                   
        ...
        # smashed_Data = [smashed_list[i][0] for i in range(len(smashed_list))]
        # smashed_Data = torch.tensor([item.cpu().detach().numpy() for item in smashed_Data])
        # smashed_labels = [smashed_list[i][1] for i in range(len(smashed_list))]
        # smashed_labels = torch.tensor([item.cpu().detach().numpy() for item in smashed_labels])
        ...
        #TODO:把LR print 出来看
        # traing evaluate ----------
        # [loss_trn, acc_trn] = local.evaluate(net = copy.deepcopy(clnt_models[clnt]).to(device), ell = args.local_ep , dataset_tst = clnt_x[clnt], dataset_tst_label = clnt_y[clnt] , net_server = net_server)
        # train_acc_all[clnt].append(acc_trn)
        # train_loss_all[clnt].append(loss_trn)
        # print('iteration', iter,'clnt_id',clnt ,'loss_trn', loss_trn, 'acc_trn', acc_trn)

        #TODO：不需要一个用户test一次？要的。测试的是client_model + server_model。
        ...
        if len(w_locals_client) >= args.K:#iter%2==0,5,10
            ...
            #TODO：重写FedBuff算法实现：不同的网络结构layer

            #TODO: AN是否要FL？ 
            # w_old = copy.deepcopy(FL_AN.to(device).state_dict())
            # AN_glob_client = FedBuff(w_old, AN_locals_client, args.async_lr) 
